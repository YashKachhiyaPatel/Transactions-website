...
function DisplayBusinessListPage(req, res, next) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            var customersBusiness = yield customer_1.default.find({ "custemail": Util_1.UserUserName(req) }, null, { sort: { bowner: 1 } }).exec();
            var result = customersBusiness.map(it => it.get("bowner"));
            var amountList = customersBusiness.map(it => it.get("custamount"));
            console.log(amountList);
            var businessList = yield business_1.default.find({ bowner: { $in: result } }, null, { sort: { bowner: 1 } }).exec();
...
function DisplayRateBusinessPage(req, res, next) {
    ...
    business_1.default.findById(id, {}, {}, (err, businessToRate) => {
        if (err) {
            console.error(err);
            res.end(err);
        }
...
function ProcessRateBusiness(req, res, next) {
    ...
    business_1.default.updateOne({ _id: id }, { $inc: { btotalrating: +rating, bnumberofratings: +1 } }, {}, (err) => {
        if (err) {
            console.error(err);
            res.end(err);
        }
